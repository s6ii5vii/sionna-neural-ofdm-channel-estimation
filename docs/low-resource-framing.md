...
## Low SNR and noisy channels

Low-SNR profiles test estimation when pilot observations are heavily corrupted
by noise. Experiments must report the SNR definition, signal-power convention,
random seed policy, and performance across a range rather than at one favorable
operating point.
...
locations and evaluates LS at those observed locations only. Full-grid
evaluation requires a documented interpolation or model-based reconstruction
method and is not yet implemented.
...
from training conditions. Candidate studies include SNR shifts, changed fading
profiles, mobility assumptions, delay spreads, and pilot patterns. These require
validated Sionna configurations before results can be interpreted.
...
- It does not yet model a complete coded OFDM link or standardized deployment.
